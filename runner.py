...
def main():
    ...
    if args.table:
        logger.warning(
            f'{cl.RED}Параметр `--table` удален - попробуйте улучшенный вывод по умолчанию, или воспользуйтесь параметром `--debug`{cl.RESET}'
        )
        print()

    if args.debug:
        logger.warning(
            f'{cl.CYAN}Параметр `--debug` больше не нужен для обычного использования – попробуйте улучшенный вывод по умолчанию убрав параметр --debug{cl.RESET}'
        )
        print()
